...
def plot_final():
    ...
    trajLength = len(robot_trajectory)
    
   # Slice target_trajectory to match the length of robot_trajectory
    ax.plot([p['x'] for p in target_trajectory[:trajLength]], [p['y'] for p in target_trajectory[:trajLength]], 
            lw=2, marker='o', color='r', label='target')
    
    ax.plot([p['x'] for p in robot_trajectory], [p['y'] for p in robot_trajectory], 
            lw=2, marker='o', color='b', label='robot')
